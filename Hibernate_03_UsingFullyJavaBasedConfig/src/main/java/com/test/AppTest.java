...
public class AppTest {
	public static void main(String[] args) {
		...
		Drinks   d1 = new Drinks(1,"maza",20,500);
		...
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			...
			List<Drinks> drinks = session.createQuery("from Drinks", Drinks.class).list();
			...
			drinks.forEach(s -> System.out.println("Drinkname>>> " + s.getdrinkname() + " Drinkprize>>> "
					+ s.getdrinkprize() + " Drinkquantity>>> " + s.getdrinkquqntity()));
